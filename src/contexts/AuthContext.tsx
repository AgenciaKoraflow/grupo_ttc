...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string): Promise<LoginResult> => {
    ...
    if (!profile) {
      ...
      return {
        success: false,
        error: 'Perfil de usuário não encontrado. Contate o administrador do sistema.',
      };
    }
  ...
  const changePassword = useCallback(async (newPassword: string): Promise<LoginResult> => {
