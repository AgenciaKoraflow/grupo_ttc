...
function TiposServicoPage() {
  ...
  const stats = useMemo(() => {
    ...
    const comServicos = tiposServico.filter(ts => servicos.some(s => s.tipo_servico_id === ts.id)).length;
  ...
  const tipoStats = useMemo(() => {
    return tiposServico.map(ts => {
      ...
      const ocs = ocorrencias.filter(o => tsServicos.some(s => s.ocorrencia_id === o.id));
      return {
        id: ts.id,
  ...
  const getTipoStats = (tipoId: string) => tipoStats.find(t => t.id === tipoId);
    ...
    <AppLayout>
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Cadastre e gerencie os tipos de serviços realizados nas ocorrências
            </p>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <Label>Nome</Label>
                <Input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex: Troca de Lâmpada" />
              </div>
              <DialogFooter>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          <StatCard label="Total de Tipos" value={stats.total} icon={Wrench} color={C.primary} />
          <StatCard label="Tipos Ativos" value={stats.ativos} icon={CheckCircle} color={C.success} />
          <StatCard label="Tipos Inativos" value={stats.inativos} icon={AlertCircle} color={C.warning} />
          <StatCard label="Com Ocorrências" value={stats.comServicos} icon={Activity} color={C.muted} />
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {tiposServico.length === 0 ? (
                ...
                return (
                  <TableRow key={ts.id}>
                    <TableCell className="font-medium">{ts.nome}</TableCell>
                    <TableCell>
                      <Badge 
                        variant={ts.ativo ? 'secondary' : 'outline'} 
                        className={cn('text-xs cursor-pointer transition-colors', ts.ativo ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'text-muted-foreground')}
                    ...
                    <TableCell className="text-center">
                      <span className="text-sm font-medium">{s?.totalServicos ?? 0}</span>
                    </TableCell>
                    <TableCell className="text-center font-semibold">{s?.totalOcorrencias ?? 0}</TableCell>
                    <TableCell className="text-center">
